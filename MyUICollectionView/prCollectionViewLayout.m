...
static NSString * const LayoutCellKind = @"PhotoCell";

@interface prCollectionViewLayout()

    @property (nonatomic, strong) NSDictionary *layoutInfo;
    @property (nonatomic, strong) prLayoutGenerator* generator;

@end

@implementation prCollectionViewLayout
...
- (CGSize)collectionViewContentSize
{
  /*  NSInteger rowCount = [self.collectionView numberOfSections] / self.numberOfColumns;
    ...
    return CGSizeMake(self.collectionView.bounds.size.width, [_generator calculateBoundHeightForNumberOfItems: 20]);
}
#pragma mark - Private
...
- (CGRect)frameForAlbumPhotoAtIndexPath:(NSIndexPath *)indexPath
{
  /*  NSInteger row = indexPath.section / self.numberOfColumns;
    ...
    if (_generator == nil){
        _generator = [[prLayoutGenerator alloc] init];
    }
    
    CGRect rect = [_generator generateWithRectIndex:indexPath.row];
